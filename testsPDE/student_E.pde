...
float screenWidth=1280;  //the width of screen
float screenHeight=720;  //the height of screen

int ballX=640;  // declare the Variables to hold x position of ball
...
float paddleWidth=30;  // width of paddles
float paddleHeight=150;  //height of paddles
...
float rightPaddleX=1250;  // declares the variables to hold x position of right paddle
float rightPaddleY=570;  // declares the variables to hold y position of right paddle
...
background(0,200,0); //sets the background to a dark red
...
fill(0); //sets the paddles' colour to black
stroke(255,0,0); //sets the paddles' outline to red
...
void displayBall(){
 ...
 stroke(0);  //set the outline colour to white
  fill(255);  //set the inside white
  ellipse(ballX, ballY, ballWidth, ballHeight);  //draw ellipse
  }
  
  void displayScores(){
    
  fill(255,0,0);  // set color red
